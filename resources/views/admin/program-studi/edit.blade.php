...
<h1 class="h4 mb-3">Edit Program Studi</h1>
    ...
    <form action="{{ route('admin.program-studi.update', $programStudi->id) }}" method="POST">
      ...
      <div class="mb-3">
        ...
        <select name="jenjang" class="form-select @error('jenjang') is-invalid @enderror" required>
          ...
          @foreach(['D1','D2','D3','D4','S1','S2','S3','Profesi','Spesialis'] as $j)
            <option value="{{ $j }}" {{ old('jenjang', $programStudi->jenjang) == $j ? 'selected' : '' }}>
              {{ $j }}
            </option>
          @endforeach
        </select>
      ...
      <div class="mb-3">
        ...
        <textarea name="keterangan" rows="4"
                  ...
                  placeholder="Masukkan deskripsi atau keterangan">{{ old('keterangan', $programStudi->keterangan) }}</textarea>
        @error('keterangan')
          <div class="invalid-feedback">{{ $message }}</div>
        @enderror
      </div>
